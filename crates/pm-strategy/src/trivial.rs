...
impl BuyYesAtOpen {
    pub fn new(clip_shares: f64) -> Self {
        Self { clip_shares, fired: false }
    }
}

impl Strategy for BuyYesAtOpen {
    fn on_event(&mut self, event: &ReplayEvent, _ctx: &Ctx, _spot: &SpotHistory, _trades: &pm_types::TradeHistory) -> StrategyOutput {
...
impl Strategy for NoopStrategy {
    fn on_event(&mut self, _event: &ReplayEvent, _ctx: &Ctx, _spot: &SpotHistory, _trades: &pm_types::TradeHistory) -> StrategyOutput {
        StrategyOutput::hold()
    }
}
